...
//  ساخت کانفیگ
// ============================================================
function buildVless(address, port, host, name) {
...
function subscription(url, host) {
  ...
  for (const ip of list) {
    for (const port of ports) {
      links.push(buildVless(ip, port, host, `Phoenix-${ip}-${port}`));
    }
  }
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <p class="hint">
      ...
      SNI و Host = <b dir="ltr">${host}</b> (همان دامنه‌ی ورکر، خودکار تنظیم می‌شود).<br>
